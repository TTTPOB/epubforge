...
MinerU-based PDF → EPUB pipeline for books and academic theses.
...
Each pipeline stage writes to `work/<book_name>/0N_*` and is independently
re-runnable. Text LLM calls are cached in `work/.cache/` (key = sha256 of model +
prompt).
...
|-------|------|----------------|-------|--------|
| 1 | parse | `parse` / `--from 1` | PDF | `01_raw.zip` (original MinerU API archive); `source/source.pdf` (hardlinked/copied) |
...
Stage 1 requires `EPUBFORGE_MINERU_API_KEY` or `[mineru].api_key`. It stores the untouched
MinerU ZIP at `01_raw.zip` and reuses that archive until `--force-rerun` is supplied.
Stages 2-4 still document and consume `01_raw.json`; this boundary remains intentionally
unadapted until the MinerU archive contract receives a downstream implementation.
...
All stages and every LLM request emit INFO-level logs. Logs are written to
...
Per-request log lines include: kind (LLM), req_id, model, cache HIT/MISS, elapsed,
...
| `render-prompt` | Render a subagent prompt with current memory and patch workflow instructions |
 | `render-page` | Render a page from `source/source.pdf` to JPEG |
...
`render-page` can be used to inspect a page visually before issuing repair ops.
...
- `Provenance` — `{page, bbox, source: "llm"|"docling"|"passthrough"}`; `source="docling"` indicates mechanical Stage 3/4 output
...
- `AgentOutput.evidence_refs` and `BookPatch.evidence_refs` remain opaque references for
...
**`[mineru]` submodel:**
```
EPUBFORGE_MINERU_API_KEY                    mineru.api_key
EPUBFORGE_MINERU_BASE_URL                   mineru.base_url
EPUBFORGE_MINERU_MODEL_VERSION              mineru.model_version
EPUBFORGE_MINERU_TIMEOUT                    mineru.timeout_seconds
EPUBFORGE_MINERU_POLL_INTERVAL_SECONDS      mineru.poll_interval_seconds
EPUBFORGE_MINERU_MAX_POLLS                  mineru.max_polls
EPUBFORGE_MINERU_MAX_DOWNLOAD_BYTES        mineru.max_download_bytes
EPUBFORGE_MINERU_MAX_UNCOMPRESSED_BYTES    mineru.max_uncompressed_bytes
EPUBFORGE_MINERU_IS_OCR                     mineru.is_ocr  (1/true/yes/on = True)
EPUBFORGE_MINERU_ENABLE_FORMULA             mineru.enable_formula  (1/true/yes/on = True)
EPUBFORGE_MINERU_ENABLE_TABLE               mineru.enable_table  (1/true/yes/on = True)
EPUBFORGE_MINERU_LANGUAGE                   mineru.language
```
