...
def _make_manifest_and_activate(
    ...
    *,
    ...
    sidecars = {
        "audit_notes": str((sidecar_dir / "audit_notes.json").relative_to(work_dir).as_posix()),
        "book_memory": str((sidecar_dir / "book_memory.json").relative_to(work_dir).as_posix()),
        "evidence_index": str((sidecar_dir / "evidence_index.json").relative_to(work_dir).as_posix()),
    }
...
class TestRunAssembleFreshness:
    ...
    def test_matching_sha_allows_skip(self, tmp_path: Path, monkeypatch: Any) -> None:
        ...
        _write_docling_unit(unit_path, 1, [{"kind": "paragraph", "text": "hello", "provenance": {"page": 1, "source": "docling"}}])
        ...
        monkeypatch.setattr(asm_mod, "assemble_from_manifest", lambda *a, **kw: called.append(1) or original(*a, **kw))
        ...
        assert not called, "assemble_from_manifest should not be called when output is fresh"
    ...
    def test_vlm_to_skip_vlm_causes_reassemble(self, tmp_path: Path) -> None:
        ...
        unit_path.parent.mkdir(parents=True, exist_ok=True)
        _write_docling_unit(unit_path, 1, [{"kind": "paragraph", "text": "skip content", "provenance": {"page": 1, "source": "docling"}}])
        ...
        out = work / "05_semantic_raw.json"
        book = Book(title="Test")
        book.extraction = _make_book_extraction("art002vlm", "old_sha_from_vlm", mode="vlm")
        out.write_text(book.model_dump_json(indent=2), encoding="utf-8")
    ...
    def test_skip_vlm_to_vlm_causes_reassemble(self, tmp_path: Path) -> None:
        ...
        out = work / "05_semantic_raw.json"
        book = Book(title="Test")
        book.extraction = _make_book_extraction("art003skip", "old_sha_from_skip", mode="skip_vlm")
        out.write_text(book.model_dump_json(indent=2), encoding="utf-8")
    ...
    def test_page_filter_change_causes_reassemble(self, tmp_path: Path) -> None:
        ...
        unit_path.parent.mkdir(parents=True, exist_ok=True)
        _write_docling_unit(unit_path, 1, [{"kind": "paragraph", "text": "filtered", "provenance": {"page": 1, "source": "docling"}}])
    ...
    def test_missing_output_causes_assemble(self, tmp_path: Path) -> None:
        ...
        unit_path.parent.mkdir(parents=True, exist_ok=True)
        _write_docling_unit(unit_path, 1, [{"kind": "paragraph", "text": "first run", "provenance": {"page": 1, "source": "docling"}}])
    ...
    def test_damaged_output_causes_reassemble(self, tmp_path: Path) -> None:
        ...
        unit_path.parent.mkdir(parents=True, exist_ok=True)
        _write_docling_unit(unit_path, 1, [{"kind": "paragraph", "text": "recovered", "provenance": {"page": 1, "source": "docling"}}])
...
class TestLegacyFallbackFails:
    def test_no_active_manifest_raises(self, tmp_path: Path) -> None:
        ...
        _write_docling_unit(
            extract_dir / "unit_0000.json", 1,
            [{"kind": "paragraph", "text": "old unit"}],
        )

        from epubforge import pipeline
        from epubforge.config import Config, RuntimeSettings
        from epubforge.stage3_artifacts import Stage3ContractError
...
class TestNoAutoHeuristics:
    ...
    def test_no_footnote_pairing(self, tmp_path: Path) -> None:
        ...
        blocks = [
            {"kind": "paragraph", "text": "Main text with callout \x01①\x01.", "provenance": {"page": 1, "source": "docling"}},
            {"kind": "footnote", "callout": "①", "text": "Footnote body.", "provenance": {"page": 1, "source": "docling"}},
        ]
        work, manifest = self._make_work_and_manifest(tmp_path, [("docling_page", blocks)])

        book = assemble_from_manifest(work, manifest)

        footnotes = [b for ch in book.chapters for b in ch.blocks if isinstance(b, Footnote)]
        assert len(footnotes) == 1
        assert not footnotes[0].paired, "No footnote pairing should happen during assemble"

    def test_no_empty_callout_merge(self, tmp_path: Path) -> None:
        """Empty-callout footnotes are NOT merged into preceding footnotes."""
        blocks = [
            {"kind": "footnote", "callout": "①", "text": "First footnote incomplete", "provenance": {"page": 1, "source": "docling"}},
            {"kind": "footnote", "callout": "", "text": " continuation here.", "provenance": {"page": 1, "source": "docling"}},
        ]
        work, manifest = self._make_work_and_manifest(tmp_path, [("docling_page", blocks)])
    ...
    def test_no_continued_table_merge(self, tmp_path: Path) -> None:
        ...
        blocks = [
            {"kind": "table", "html": "<table><tr><td>A</td></tr></table>", "continuation": False, "provenance": {"page": 1, "source": "docling"}},
            {"kind": "table", "html": "<table><tr><td>B</td></tr></table>", "continuation": True, "provenance": {"page": 2, "source": "docling"}},
        ]
        work, manifest = self._make_work_and_manifest(tmp_path, [("docling_page", blocks)])
    ...
    def test_no_heading_chapter_split(self, tmp_path: Path) -> None:
        ...
        blocks = [
            {"kind": "heading", "text": "Chapter 1", "level": 1, "provenance": {"page": 1, "source": "docling"}},
            {"kind": "paragraph", "text": "Chapter 1 content.", "provenance": {"page": 1, "source": "docling"}},
            {"kind": "heading", "text": "Chapter 2", "level": 1, "provenance": {"page": 2, "source": "docling"}},
            {"kind": "paragraph", "text": "Chapter 2 content.", "provenance": {"page": 2, "source": "docling"}},
        ]
        work, manifest = self._make_work_and_manifest(tmp_path, [("docling_page", blocks)])
...
class TestSingleChapter:
    def test_docling_produces_single_chapter(self, tmp_path: Path) -> None:
        ...
        _write_docling_unit(u, 1, [{"kind": "paragraph", "text": "para", "provenance": {"page": 1, "source": "docling"}}])
        _, manifest = _make_manifest_and_activate(work, mode="skip_vlm", artifact_id="sc001", unit_file_paths=[u])
    ...
    def test_vlm_produces_single_chapter(self, tmp_path: Path) -> None:
        ...
        _, manifest = _make_manifest_and_activate(work, mode="vlm", artifact_id="sc002", unit_file_paths=[u])
...
class TestProvenanceMapping:
    def _run(self, tmp_path: Path, unit_kind: str, write_fn: Any, blocks: list[dict[str, Any]]) -> Book:
        work = tmp_path / "work" / "book"
        work.mkdir(parents=True, exist_ok=True)
        ...
        u = art_dir / "unit_0000.json"
        write_fn(u, blocks)
        mode: Literal["vlm", "skip_vlm"] = "skip_vlm" if unit_kind == "docling_page" else "vlm"
        _, manifest = _make_manifest_and_activate(work, mode=mode, artifact_id=art_id, unit_file_paths=[u])
        return assemble_from_manifest(work, manifest)

    def test_docling_page_source_is_docling(self, tmp_path: Path) -> None:
        blocks = [{"kind": "paragraph", "text": "hi", "provenance": {"page": 1, "source": "docling"}}]
        book = self._run(
            tmp_path,
            "docling_page",
...
class TestUnknownUnitKind:
    def test_unknown_unit_kind_raises(self, tmp_path: Path) -> None:
        ...
        _, manifest = _make_manifest_and_activate(work, mode="skip_vlm", artifact_id="unk001", unit_file_paths=[u])
...
class TestExtractionMetadataWritten:
    def test_extraction_metadata_fields(self, tmp_path: Path) -> None:
        ...
        art_dir.mkdir(parents=True, exist_ok=True)
        u = art_dir / "unit_0000.json"
        _write_docling_unit(u, 1, [{"kind": "paragraph", "text": "meta test", "provenance": {"page": 1, "source": "docling"}}])
        ...
        assert book.extraction.evidence_index_path == manifest.sidecars["evidence_index"]
