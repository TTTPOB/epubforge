...
_BASE_SETTINGS_DOCLING: dict[str, Any] = {
    "contract_version": 3,
...
class TestRunAssembleFreshness:
    ...
    def test_matching_sha_allows_skip(self, tmp_path: Path, monkeypatch: Any) -> None:
        ...
        pointer, manifest = _make_manifest_and_activate(
            work, artifact_id="art001", unit_file_paths=[unit_path]
        )
    ...
    def test_artifact_id_mismatch_causes_reassemble(self, tmp_path: Path) -> None:
        """Different artifact_id in output vs active manifest causes reassemble."""
        cfg_work, work = self._setup_work_dir(tmp_path, stem="book")
        unit_path = work / "03_extract" / "artifacts" / "art002new" / "unit_0000.json"
        unit_path.parent.mkdir(parents=True, exist_ok=True)
        _write_docling_unit(
            unit_path,
            1,
        ...
        pointer, manifest = _make_manifest_and_activate(
            work, artifact_id="art002new", unit_file_paths=[unit_path]
        )

        # Write stale output pretending it was generated by old artifact "art002old"
        out = work / "05_semantic_raw.json"
        book = Book(title="Test")
        book.extraction = _make_book_extraction("art002old", "old_sha")
        out.write_text(book.model_dump_json(indent=2), encoding="utf-8")
        ...
        # Output should be regenerated with the new artifact
        result = Book.model_validate_json(out.read_text(encoding="utf-8"))
        assert result.extraction.artifact_id == "art002new"
        assert result.extraction.stage3_mode == "docling"
        assert result.extraction.stage3_manifest_sha256 == pointer.manifest_sha256

    def test_sha_mismatch_causes_reassemble(self, tmp_path: Path) -> None:
        """Same artifact_id but different manifest sha256 causes reassemble."""
        cfg_work, work = self._setup_work_dir(tmp_path, stem="book")
        unit_path = work / "03_extract" / "artifacts" / "art003same" / "unit_0000.json"
        unit_path.parent.mkdir(parents=True, exist_ok=True)
        _write_docling_unit(
            unit_path,
            1,
        ...
        pointer, manifest = _make_manifest_and_activate(
            work, artifact_id="art003same", unit_file_paths=[unit_path]
        )

        # Write stale output with same artifact_id but stale sha256
        out = work / "05_semantic_raw.json"
        book = Book(title="Test")
        book.extraction = _make_book_extraction("art003same", "stale_sha_from_old_run")
        out.write_text(book.model_dump_json(indent=2), encoding="utf-8")
        ...
        result = Book.model_validate_json(out.read_text(encoding="utf-8"))
        assert result.extraction.artifact_id == "art003same"
        assert result.extraction.stage3_mode == "docling"
        assert result.extraction.stage3_manifest_sha256 == pointer.manifest_sha256
    ...
    def test_page_filter_change_causes_reassemble(self, tmp_path: Path) -> None:
        ...
        pointer, _ = _make_manifest_and_activate(
            work, artifact_id="art004new", unit_file_paths=[unit_path]
        )
    ...
    def test_missing_output_causes_assemble(self, tmp_path: Path) -> None:
        ...
        _make_manifest_and_activate(
            work, artifact_id="art005", unit_file_paths=[unit_path]
        )
    ...
    def test_damaged_output_causes_reassemble(self, tmp_path: Path) -> None:
        ...
        _make_manifest_and_activate(
            work, artifact_id="art006", unit_file_paths=[unit_path]
        )
...
class TestNoAutoHeuristics:
    def _make_work_and_manifest(
        ...
        _, manifest = _make_manifest_and_activate(
            work, artifact_id=art_id, unit_file_paths=unit_paths
        )
...
class TestSingleChapter:
    def test_docling_produces_single_chapter(self, tmp_path: Path) -> None:
        ...
        _, manifest = _make_manifest_and_activate(
            work, artifact_id="sc001", unit_file_paths=[u]
        )
    ...
    def test_vlm_produces_single_chapter(self, tmp_path: Path) -> None:
        ...
        _, manifest = _make_manifest_and_activate(
            work, artifact_id="sc002", unit_file_paths=[u]
        )
...
class TestProvenanceMapping:
    def _run(
        ...
        write_fn(u, blocks)
        _, manifest = _make_manifest_and_activate(
            work, artifact_id=art_id, unit_file_paths=[u]
        )
...
class TestUnknownUnitKind:
    def test_unknown_unit_kind_raises(self, tmp_path: Path) -> None:
        ...
        _, manifest = _make_manifest_and_activate(
            work, artifact_id="unk001", unit_file_paths=[u]
        )
