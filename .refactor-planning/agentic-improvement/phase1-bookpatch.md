...
`old` 与 `new` 的类型注解改为 `Any`（从 `object`），并在文档中明确序列化约定：
...
`ReplaceNodeChange` 仅用于 block 级别的替换，不用于替换 chapter。chapter metadata 修改通过 `SetFieldChange`（`title`、`level`、`id` 字段），chapter 结构变更通过 `delete_node` + `insert_node` 组合。这样避免了"替换 chapter 时如何处理 blocks 列表"的歧义语义。
...
InsertNodeChange 的内部 Pydantic 校验路径：
...
IRChange union 是封闭的五种原子类型。batch change（如 `move_block_range`）通过 Phase 3 的 PatchCommand macro 实现（编译为多个 MoveNodeChange），不在 IRChange 层面扩展。这是有意的设计决策，避免 IRChange union 随 macro 需求不断膨胀。
...
恢复了 `table.bbox` 字段（`semantic.py` 第 149 行 `Table` 模型含 `bbox: list[float] | None = None`，此前遗漏）。
...
# Explicit heading level constraint — Heading.level in IR is int (no Literal
...
# Chapter.level is also int with no range constraint in IR — enforce here.
_VALID_CHAPTER_LEVELS: frozenset[int] = frozenset({1, 2, 3})
```

`Heading.level` 在 `semantic.py` 中声明为 `int = 1`（无 Literal 约束，第 97 行），`Chapter.level` 同理（第 171 行）。不能依赖 IR Pydantic 模型来约束 level 范围——`patches.py` 必须通过 `_VALID_HEADING_LEVELS` 和 `_VALID_CHAPTER_LEVELS` 显式检查。`ops.py` 的 `HeadingPayload` 使用 `Literal[1, 2, 3]`（第 72 行）是 EditOp 层的约束，不影响 IR 层。

---

## 4. Apply 逻辑（事务性 validate+apply 合并）

原计划将 `validate_book_patch` 和 `apply_book_patch` 拆为两个独立函数，但评审 D2 和 T4 共同揭示了一个根本设计问题：
...
`semantic.py` 第 82 行 `_UidMixin.uid: str | None = None` 和第 169 行 `Chapter.uid: str | None = None` 表明 Book IR 中合法存在 `uid=None` 的节点。BookPatch 系统无法为无 uid 的节点建立索引，因此在 apply 入口处强制检查：
...
#### 步骤 3：逐 change 静态检查（不含 precondition）
...
precondition 检查在每个 change 执行前、针对 `working`（已应用前序 changes 的副本）进行，而非原始 book。这使链式修改成为可能。
...
2. 若 `field == "level"` 且 block kind == "heading"：验证 `change.new in _VALID_HEADING_LEVELS`
3. 若 `field == "level"` 且 target 是 chapter：验证 `change.new in _VALID_CHAPTER_LEVELS`
...
每次 `_apply_change` 执行后，按以下策略决定是否重建索引：
...
在 `apply_book_patch` 的步骤 5 完成后调用。
...
`_make_block` 是 Phase 1 新写的辅助函数，不从 `apply.py` 引入（`apply.py` 中并无对应的独立 `_make_block` 函数，它直接调用 `Paragraph(...)`、`Heading(...)` 等构造器）。`_make_block` 的实现逻辑：根据 `kind` 查找对应的 IR Block 类（`Paragraph`、`Heading` 等），将 `uid` 注入 `data` 后调用 `Model.model_validate(data)`。
...
| 修改 heading.level（1→5，非法值） | PatchError：new value 不合法（_VALID_HEADING_LEVELS 检查） |
| 修改 chapter.level（1→4，非法值） | PatchError：new value 不合法（_VALID_CHAPTER_LEVELS 检查） |
...
| 修改 chapter.title | 成功 |
| 修改 table.html | 成功 |
| 修改 table.bbox | 成功（table.bbox 已加入 _ALLOWED_SET_FIELD） |
| `target_uid` 为纯空白字符串 | Pydantic 模型层 ValueError（require_non_empty） |
| `field` 为纯空白字符串 | Pydantic 模型层 ValueError（require_non_empty） |
...
| `old_node` 不匹配当前状态 | PatchError：old_node precondition mismatch（model_dump(mode="python") 比较） |
| `new_node` 包含 uid 键 | Pydantic 模型层 ValueError |
| `new_node` 缺少 kind 键 | Pydantic 模型层 ValueError |
| `new_node` 的 kind 合法但 level=5（heading） | PatchError：new_node Payload 校验失败（HeadingPayload.level Literal[1,2,3]） |
| target_uid 不存在 | PatchError：target_uid not found |
...
| 插入新 chapter（parent_uid=None，含 title，blocks 为空） | 成功 |
| 插入新 chapter（parent_uid=None，含 blocks） | 成功，blocks 一并写入 |
...
| `old_node` 不匹配当前状态 | PatchError：old_node precondition mismatch |
| target_uid 不存在 | PatchError：target_uid not found |
...
| 移动到目标章的特定位置 | 成功，位置正确 |
| 章内移动到当前位置（no-op move） | 成功，apply 后 book 内容不变，版本 +1 |
| chapter 在 book 内移动（from_parent_uid=None, to_parent_uid=None） | 成功，chapter 顺序更新 |
...
| `agent_id` 为纯空白字符串 | Pydantic 模型层 ValueError（require_non_empty 使用 .strip()） |
| `rationale` 为空字符串 | Pydantic 模型层 ValueError |
| `rationale` 为纯空白字符串 | Pydantic 模型层 ValueError |
...
| **对同一节点的链式 SetFieldChange** | 见下方详细说明 |

链式 SetFieldChange 测试：
...
| heading level 通过 set_field 设为非法值（5） | PatchError（_VALID_HEADING_LEVELS 检查） |
| heading level 通过 replace_node 新节点设为非法值（5） | PatchError（HeadingPayload Payload 校验，Literal[1,2,3]） |

### 8.13 ReplaceNodeChange 与 Chapter 相关测试

按 §2.4.2 的设计，`ReplaceNodeChange` 不用于 chapter 替换。因此：
...
- **Phase 3**：每个 `PatchCommand` macro 的 `compile()` 方法返回 `BookPatch`；编译后立即调用 `apply_book_patch` 确认语义。batch change（如 `move_block_range`、`split_chapter`、`merge_chapters`）均走 macro 路线，不扩展 IRChange union。
- **Phase 4**：删除旧 `ops.py` / `apply.py` 时，确认 `patches.py` 的 `_make_block` 是唯一 block 构造入口。uid=None 的 IR 字段在此阶段可考虑改为 `str`（必填），消除 BookPatch 的 uid 守卫前置条件。
- **Phase 6**：`diff_books(base, proposed)` 的返回值是 `BookPatch`；`apply_book_patch(base, diff_books(base, proposed)) == proposed` 是 round-trip 不变量。注意 diff_books 生成的 BookPatch 中，old/new 值通过 `model_dump(mode="json")` 序列化，与 precondition 比较约定一致。
